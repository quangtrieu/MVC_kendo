...
#pragma warning restore 612, 618
        ...
        public T Get<T>(string name)
        {
            ...
            if (UseCache)
            {
                var keyOuput = GenerateKeyCacheOutput(KeyOutParams, name);
                var hashKey = Md5EnCrypt(keyOuput);
                val = Cache.Get(hashKey);
                if (val == null)
                {
                    val = parameters[Clean(name)].AttachedParam.Value;
                    Cache.Store(StoreMode.Add, hashKey, val);
                }
            }
            else
            {
