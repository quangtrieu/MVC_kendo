...
using BCS.Framework.Helper.Attributes;
using BCS.Framework.Helper.Cache;
using BCS.Framework.Security.Crypt;
...
namespace BCS.Framework.Data
{
    ...
    public class ADOProvider : IDisposable
    {
        ...
        public IDbConnection Connection
        {
            get
            {
                ...
                if (ConnectionStringKey == null)
                    return new SqlConnection(DataHelper.GetConnectionString());
                if (ConnectionStringKey == Constant.DATABASE_CURRENT && !string.IsNullOrEmpty(_schema))
                {
                    return new SqlConnection(_schema);
                }

                return new SqlConnection(DataHelper.GetConnectionString(ConnectionStringKey));
            }
        }
        public void SetWriteConnectionString(bool writeFlag)
        {
            if (writeFlag)
                this.ConnectionString = Utils.GetSetting<string>("BCSWrite");
            else
                this.ConnectionString = Utils.GetSetting<string>("BCSRead");
        }
        ...
        {
            _schema = schema;
            ConnectionStringKey = schema;
            //ConnectionStringKey = "BCSConnection";
        }
        /// <summary>
        /// Tenants the internal.
        /// </summary>
        /// <param name="schema">The schema.</param>
        /// <returns></returns>
        public ADOProvider(string schema, string connectionKey)
        {
            _schema = schema;
